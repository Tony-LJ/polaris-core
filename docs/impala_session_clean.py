...
"""
descr : Impala Conn Session定时清理脚本
auther : lj.michale
create_date : 2025/10/10 15:54
file_name : impala_session_clean.py
"""
...
# 配置ip地址，格式为
ips = ["10.53.0.71", "10.53.0.72", "10.53.0.73", "10.53.0.74", "10.53.0.75"]
...
for ip in ips:
    ...
    c_tr = oup.find_all("table")[0].find_all("tr")
    ...
    for i in range(len(c_tr)):
        if len(c_tr[i].find_all("td")):
            # 判断session是否显示为已经过期
            if 'true' in str(c_tr[i].find_all("td")[11]):
                detail_list.append(c_tr[i].find_all("td")[14])  # 清理的程序
...
print("Impala Conn Session已被清理成功!")
